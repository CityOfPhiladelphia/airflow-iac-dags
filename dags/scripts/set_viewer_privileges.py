...
def set_viewer_privileges(**kwargs):
    ...
    ######################
    # Connect to Postgres
    pg_db_conn = BaseHook.get_connection(postgres_conn_id)
    pg_conn_kwargs = {
        'user': pg_db_conn.login,
        'password': pg_db_conn.password,
        'host': pg_db_conn.host,
        'port': pg_db_conn.port,
        'database': json.loads(pg_db_conn.extra)['db_name']
        }
    pg_conn = psycopg2.connect(**pg_conn_kwargs)
    ...
    for c in current_grantees_list:
        if c not in wanted_grantees_list:
            privilege_update_stmt.append(f'REVOKE ALL PRIVILEGES ON TABLE {viewer_account}.{table_name} FROM {c};')
    
    for w in wanted_grantees_list:
        if w not in current_grantees_list and w not in all_priv_grantees:
            privilege_update_stmt.append(f'GRANT SELECT ON TABLE {viewer_account}.{table_name} TO {w};')

    for a in all_priv_grantees:
        if a not in current_grantees_list:
            privilege_update_stmt.append(f'GRANT ALL PRIVILEGES ON TABLE {viewer_account}.{table_name} TO {a};')
    ...
    if current_owner != viewer_account:
        privilege_update_stmt.append(f'ALTER TABLE {viewer_account}.{table_name} OWNER TO {viewer_account};')
