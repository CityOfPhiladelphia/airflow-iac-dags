...
def update_metadata_data_last_updated(
    user, dataset, conn, dest_schema, date, dry_run=False
):
    """
    ...
    if user.lower().startswith("viewer_"):
        ...
        possible_names.append(
            f"databridge.{user.split('viewer_')[1].lower()}.{dataset.lower()}"
        )
    else:
    ...
    if not RECORD_ID:
        ...
        print(
            f'Found metadata.phila.gov entry! Trying to update data_last_updated field ({FIELD_ID}) in "{OBJECT_ID}" (Representations) in Knack.'
        )
        ...
        if dry_run:
            print(
                f"Dry run mode, would have updated metadata against this record: {API_URL}"
            )
        else:
            ...
            data = {
                FIELD_ID: date.strftime(
                    "%Y-%m-%dT%H:%M:%SZ"
                )  # Replace with the correct field key and value
            }
...
def update_ago_data_last_updated(
    table_name, ago_user, date, alternate_upload_name=None, dry_run=False
):
    """
    ...
    if not itemid:
        ...
        params = {
            "q": f"title:{table_name_to_search}",
            "f": "json",
        ...
        if results["total"] == 0:
            ...
            print(
                f"Found itemid by searching against the /sharing/rest/search endpoint!"
            )
    ...
    if dry_run:
        print(
            f"Dry run mode, would have updated AGO metadata revisedDate for this item: {itemid}"
        )
    else:
...
def update_postgres_tracker_table(
    account_name,
    table_name,
    dest_schema,
    conn_id,
    upload_to_knack,
    upload_to_knack_dry_run,
    upload_to_ago,
    ...
    if True:  # Temp added line to keep spacing consistent
        ...
        xcom_xmin_key = (
            f"{account_name.lower()}_{table_name.lower()}__{dag_run_id}_XMIN"
        )
        xcom_row_count_key = (
            f"{account_name.lower()}_{table_name.lower()}__{dag_run_id}_ROW_COUNT"
        )
        # Also grab our "tables different" xcom key.
        xcom_table_different_key = (
            f"{account_name.lower()}_{table_name.lower()}__{dag_run_id}_TABLE_DIFFERENT"
        )
        ...
        if tables_different == "false":
            print(
                "Tables detected as being the same, won't attempt to update Metadata.phila.gov's \"data_last_updated\" field in Knack.\n"
            )
        if tables_different == "true":
            if upload_to_knack:
                update_metadata_data_last_updated(
                    account_name.lower(),
                    table_name.lower(),
                    conn,
                    dest_schema,
                    timestamp,
                    dry_run=upload_to_knack_dry_run,
                )
            else:
                print("Skipping Knack upload")
            if upload_to_ago:
                update_ago_data_last_updated(
                    table_name.lower(),
                    ago_user,
                    timestamp,
                    ago_alternate_upload_name,
                    dry_run=upload_to_ago_dry_run,
                )
            else:
                print("Skipping AGO upload")
        ...
        try:
            with conn:
                with conn.cursor() as cur:
                    print(
                        "Updating xmin in citygeo.airflow_xmin_change_history table.."
                    )
